...
const {
  ...
  getCardMoves,
  calculateDamage,
} = require("./glogic.js"); // Assuming you have implemented the duel logic in gamelogic.js
  ...
  async execute(client, message, args, interaction) {
    ...
    try {
      ...
      async function sendDuelStatus(
        ...
        const filter = (i) =>
          ((i.user.id === message.author.id || i.user.id === opponent) &&
          ...
          collector.on("collect", async (interaction) => {
            ...
            if (attackedUsers.includes(interaction.user.id)) {
              ...
              await interaction.followUp({
                content: "You have already attacked lol",
                ephemeral: true,
              });
              return;
            }
            ...
            if (
              ...
              !attackedUsers.includes(interaction.user.id)
            ) {
              ...
              if (interaction.customId === "action_attack") {
                ...
                opponentHP -= damage;
                battleLogs.push(
                  `${authorUserName} attacks for ${damage} damage!`
                );
                attackedUsers = [];
                currentTurn = [];
                ...
                const movesArray =
                  attackedUsers[0] === opponent
                    ? authorMoves[0]
                    : opponentMoves[0];
                console.log(
                  "thisattackedusers[0]insidesendDUel:",
                  attackedUsers[0]
                );
                ...
                const selectedMove = movesArray.find(
                  (move) => move.id.toString() === selectedMoveId
                );
                ...
                if (selectedMove) {
                  ...
                  const abilityDamage = calculateAbilityDamage(
                    selectedMove.power
                  );
                  if (abilityDamage > 0) {
                    ...
                    battleLogs.push(
                      `${authorUserName} uses ${ability.name} for ${abilityDamage} damage!`
                    );
                  } else if (abilityDamage < 0) {
                    authorHP -= abilityDamage;
                    const heal = -1 * abilityDamage;
                    battleLogs.push(
                      `${authorUserName} uses ${ability.name} to heal ${heal} amount fr!`
                    );
                  }
                  attackedUsers = [];
              ...
              if (interaction.customId === "action_attack") {
                ...
                attackedUsers.push(interaction.user.id);
                battleLogs.push(
                  `${opponentUserName} attacks for ${damage} damage!`
                );
              } else if (interaction.customId === "action_dodge") {
                ...
                if (selectedMove) {
                  ...
                  if (abilityDamage > 0) {
                    ...
                    authorHP -= abilityDamage;
                    battleLogs.push(
                      `${opponentUserName} uses ${ability.name} for ${abilityDamage} damage!`
                    );
                  } else if (abilityDamage < 0) {
                    opponentHP -= abilityDamage;
                    const heal = -1 * abilityDamage;
                    battleLogs.push(
                      `${opponentUserName} uses ${ability.name} to heal ${heal} amount fr!`
                    );
                  }
                  console.log("abilitydamage:", abilityDamage);
    ...
    opponentUserName, authorCards, authorHP, opponent, opponentCards, opponentHP, battleLogs);
          message.channel.send({ embeds: [finalEmbed] });*/
              ...
              const updatedEmbed = generateDuelStatusEmbed(
                author,
                authorUserName,
                opponentUserName,
                authorCards,
            ...
            if (reason === "time") {
              ...
              if (currentTurn?.includes(author)) {
                ...
                message.channel.send(
                  `${authorUserName} took too long to respond. Skipping ${authorUserName}'s turn.`
                );
                ...
                message.channel.send(
                  `${opponentUserName} took too long to respond. Skipping ${opponentUserName}'s turn.`
                );
              }
              collector.stop();
              ...
              if (collectorStopped === true) {
                ...
                const authorPadding = " ".repeat(
                  Math.max(0, maxPaddingLength - authorFamiliarText.length)
                );
                const opponentPadding = " ".repeat(
                  Math.max(0, maxPaddingLength - opponentFamiliarText.length)
                );
...
function getDuelActionRow(
  ...
  const movesArray =
    attackedUsers[0] === opponent ? authorMoves[0] : opponentMoves[0];
...
function generateDuelStatusEmbed(
  author,
  authorUserName,
  opponentUserName,
  authorCards,
  ...
  const opponentPaddingTwo = " ".repeat(
    Math.max(0, mPL - opponentUserName.length)
  );
