  ...
  const { Location, Floor, floor1, floor2, allFloors } = require('../information/loc.js');
  ...
  const questData = require('../quest/quests.js');
  const bossData = require('../monsterInfo/bosses');
const { GameImage, Player, Element } = require('./sumfunctions.js');
    const fs = require('fs');
  let selectedValue = [];
...
// Define the filter based on the _id
  const dbFilter = { _id: message.author.id };
      ...
      const attachment = new AttachmentBuilder('./commands/adv/information/area2.png', 'area2.png');
